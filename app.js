...
client.on('voiceStateUpdate', async (oldState, newState) => {
    ...
    if (newState.channelId === trackChannelId && !oldState.channelId) {
        try {
            ...
            //บอกว่าห้องนี้มีจำนวนคนที่อยู่ในห้อง
            await newChannel.send(`🟢 **จำนวนคนที่อยู่ในห้อง:** ${newChannel.members.size} คน`);
            // เรียกบอทเข้าห้อง TTS
            await newChannel.send(`/join`);
            ...
            await alertChannel.send(`[กำลังทดสอบระบบ] <@${newState.member.displayName}> ได้ทำการไลฟ์สตรีมแล้วที่ Tiktok ห้องพูดคุย **${newChannel.name}**`);
        } catch (error) {
